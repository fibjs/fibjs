...
assert.deepEqual(a.concat(new collection.List()).toArray(), [ 1, 2, 3 ]);
assert.deepEqual(a.concat([ 4, 5, 6 ]).toArray(), [ 1, 2, 3, 4, 5, 6 ]);
assert.deepEqual(a.concat(a, [ 4, 5, 6 ], a).toArray(), [ 1, 2, 3, 1, 2, 3, 4, 5,
		6, 1, 2, 3 ]);
...
assert.deepEqual(passed.filter(isBigEnough).toArray(), [ 12, 130, 44 ]);
