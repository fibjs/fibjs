...
describe(
		...
		function() {
			it(
					...
					function() {
						...
						assert.equal(encoding.base64Decode('Kioq').toString(),
								'***');
						assert.equal(encoding.base64Decode('KioqKg==')
								.toString(), '****');
						assert.equal(encoding.base64Decode('KioqKio=')
								.toString(), '*****');
						assert.equal(encoding.base64Decode('KioqKioq')
								.toString(), '******');
						assert.equal(encoding.base64Decode('KioqKioqKg==')
								.toString(), '*******');
						assert.equal(encoding.base64Decode('KioqKioqKio=')
								.toString(), '********');
						assert.equal(encoding.base64Decode('KioqKioqKioq')
								.toString(), '*********');
						assert.equal(encoding.base64Decode('KioqKioqKioqKg==')
								.toString(), '**********');
						assert.equal(encoding.base64Decode('KioqKioqKioqKio=')
								.toString(), '***********');
						assert.equal(encoding.base64Decode('KioqKioqKioqKioq')
								.toString(), '************');
						assert.equal(encoding.base64Decode(
								'KioqKioqKioqKioqKg==').toString(),
								'*************');
						assert.equal(encoding.base64Decode(
								'KioqKioqKioqKioqKio=').toString(),
								'**************');
						assert.equal(encoding.base64Decode(
								'KioqKioqKioqKioqKioq').toString(),
								'***************');
						assert.equal(encoding.base64Decode(
								'KioqKioqKioqKioqKioqKg==').toString(),
								'****************');
						assert.equal(encoding.base64Decode(
								'KioqKioqKioqKioqKioqKio=').toString(),
								'*****************');
						assert.equal(encoding.base64Decode(
								'KioqKioqKioqKioqKioqKioq').toString(),
								'******************');
						assert.equal(encoding.base64Decode(
								'KioqKioqKioqKioqKioqKioqKg==').toString(),
								'*******************');
						assert.equal(encoding.base64Decode(
								'KioqKioqKioqKioqKioqKioqKio=').toString(),
								'********************');
						...
						assert.equal(encoding.base64Decode('Kio').toString(),
								'**');
						assert.equal(
								encoding.base64Decode('KioqKg').toString(),
								'****');
						assert.equal(encoding.base64Decode('KioqKio')
								.toString(), '*****');
						assert.equal(encoding.base64Decode('KioqKioqKg')
						...
						assert.equal(encoding
								.base64Decode('KioqKioqKioqKioqKg').toString(),
								'*************');
						assert.equal(encoding.base64Decode(
								'KioqKioqKioqKioqKio').toString(),
								'**************');
			...
			it('uri', function() {
				...
				assert.equal(encodeURIComponent(u), encoding
