...
function printSimpleMyersDiff(diff) {
  ...
  for (let diffIdx = diff.length - 1; diffIdx >= 0; diffIdx--) {
    ...
    if (type === 'insert') {
      message += `${colors.green}${value}${colors.clear}`;
    } else if (type === 'delete') {
      message += `${colors.red}${value}${colors.clear}`;
    } else {
      message += `${value}`;
    }
  }
  ...
  for (let diffIdx = diff.length - 1; diffIdx >= 0; diffIdx--) {
    ...
    if (typeChanged && previousType === 'nop') {
      ...
      if (nopCount === kNopLinesToCollapse + 1) {
        ...
        message += `${colors.blue}...${colors.clear}\n`;
        message += `  ${diff[diffIdx + 1].value}\n`;
        skipped = true;
      }
      nopCount = 0;
    ...
    if (type === 'insert') {
      message += `${colors.green}+${colors.clear} ${value}\n`;
    } else if (type === 'delete') {
      message += `${colors.red}-${colors.clear} ${value}\n`;
    } else if (type === 'nop') {
