...
class XmlText : public XmlNodeMixin<XmlText, XmlText_base> {
public:
    XmlText(XmlDocument_base* document, const char* data)
        : XmlNodeMixin<XmlText, XmlText_base>(document, xml_base::C_TEXT_NODE)
        , m_data(data)
    {
    }
    ...
    XmlText(const XmlText& from)
        : XmlNodeMixin<XmlText, XmlText_base>(from)
        , m_data(from.m_data)
    {
    }
