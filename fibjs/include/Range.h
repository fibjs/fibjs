...
public:
    Range(obj_ptr<SeekableStream_base> stream, int64_t begin, int64_t end)
    {
        m_stream = stream;
        b_pos = begin;
        e_pos = end;
...
public:
    virtual result_t read(int32_t bytes, obj_ptr<Buffer_base>& retVal)
    {
        ...
        if (bytes < 0 || bytes > rest_sz) {
            hr = m_stream->cc_read(rest_sz, retVal);
        } else {
            hr = m_stream->cc_read(bytes, retVal);
        }
    ...
    virtual result_t current(int64_t& retVal)
    {
    ...
    virtual result_t readAll(obj_ptr<Buffer_base>& retVal)
    {
        return this->read(-1, retVal);
    }
...
public:
    ...
    virtual result_t slice(obj_ptr<Buffer_base>& retVal)
    {
        retVal = new Buffer();
    ...
    virtual result_t clone(obj_ptr<SeekableStream_base>& retVal, AsyncEvent* ac)
    {
        obj_ptr<Buffer_base> buf = new Buffer();
        retVal = new MemoryStream();

        int64_t curpos = real_pos;
        rewind();

        result_t hr = slice(buf);
        real_pos = curpos;
        if (hr < 0)
            return hr;

        retVal = new MemoryStream();
        hr = retVal->write(buf, ac);
        retVal->rewind();
