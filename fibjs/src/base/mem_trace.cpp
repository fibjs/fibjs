...
#ifdef i386
#define get_bp(bp) asm("movl %%ebp, %0" \
                       : "=r"(bp)       \
                       :)
#endif

#ifdef amd64
#define get_bp(bp) asm("movq %%rbp, %0" \
                       : "=r"(bp)       \
                       :)
#endif
...
public:
    void save(size_t sz, void* bp)
    {
        ...
        if ((((intptr_t)frame & stack_align_mask) != 0) || (st && frame >= st) || frame->ret == 0)
            return;
        ...
        while (frame && m_frame_count < (int32_t)ARRAYSIZE(m_frames)) {
            ...
            if (frame1 < frame || (((intptr_t)frame1 & stack_align_mask) != 0) || (st && frame1 >= st) || frame1->ret == 0)
                break;
            frame = frame1;
        }
    }
