...
static void printHelp()
{
    puts("Usage: fibjs [options] [script.js] [arguments] \n"
        ...
        "    --update [pkg]            resolve again to the newest the ranges allow\n"
        "                              (the whole project, or that package and what\n"
        "                              it needs) and write the lockfile. package.json\n"
        "                              is not touched.\n"
        "    --ci, --frozen-lockfile   install exactly what the lockfile says; fail\n"
        "                              when it does not match package.json; write\n"
        "                              nothing. With --dry-run it only checks.\n"
        "    --lockfile-only           update the lockfile without installing.\n"
        ...
        "    --production              same as --omit=dev (npm compatible).\n"
        "    --no-audit --no-fund --force --legacy-peer-deps --silent\n"
        "                              accepted for npm compatibility, ignored.\n"
        "    unknown options           rejected, they used to be ignored silently.\n"
        "\n"
        "    devDependencies are installed by default, like npm, and NODE_ENV=\n"
        "    production means --omit=dev. package-lock.json is read and written the\n"
        "    way npm does it (npm-shrinkwrap.json wins over it). Resolving keeps the\n"
        "    versions the lockfile pins; --update is how the newest the ranges allow\n"
        "    is asked for. Unlike npm,\n"
        "    `fibjs --install <pkg>` records nothing in package.json unless --save\n"
        "    or --save-dev is given.\n"
        "\n"
