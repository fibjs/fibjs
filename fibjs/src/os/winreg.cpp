...
public:
	...
	result_t parse(exlib::string key, HKEY& root, exlib::string& path) 
	{
		...
		for (int i = 0; i < 6; i++) {
			rnz = strlen(s_root_name1[i]);
			if (0 == qstricmp(k.c_str(), s_root_name1[i], rnz)) {
				root = s_root_hkey[i];
				path = k.substr(rnz);
				break;
			}
			rnz = strlen(s_root_name2[i]);
			if (0 == qstricmp(k.c_str(), s_root_name2[i], rnz)) {
				root = s_root_hkey[i];
				path = k.substr(rnz);
				break;
			}
...
public:
	Windows::HKEY   hKey;  // opended handler key
	exlib::wstring  sKey;  // opended handler key(without Root)
};
...
result_t winreg_base::delKey(exlib::string key)
{
	...
	LONG lResult = RegDeleteTreeW(root, wpath.c_str());
