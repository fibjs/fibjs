...
{
    uint64_t info;
    int32_t which[] = { CTL_HW, HW_MEMSIZE };
    size_t size = sizeof(info);
...
result_t os_base::cpus(v8::Local<v8::Array>& retVal)
{
    ...
    uint32_t ticks = (uint32_t)sysconf(_SC_CLK_TCK), multiplier = ((uint64_t)1000L / ticks);
    char model[512];
    uint64_t cpuspeed;
    size_t size;
    ...
    for (uint32_t i = 0; i < numcpus; i++) {
        ...
        cputimes->Set(isolate->NewString("irq"), v8::Number::New(isolate->m_isolate, 0));

        cpuinfo->Set(isolate->NewString("model"), isolate->NewString(model));
        cpuinfo->Set(isolate->NewString("speed"),
