...
static void call_method(const v8::FunctionCallbackInfo<v8::Value>& args)
{
    ...
    v8::Local<v8::Object> o = v8::Local<v8::Object>::Cast(JSValue(_data->Get(context, NewString(isolate, "obj"))));
    v8::Local<v8::Function> func = v8::Local<v8::Function>::Cast(JSValue(o->Get(context, NewString(isolate, "executeDevToolsMethod"))));

    std::vector<v8::Local<v8::Value>> _args;

    _args.resize(args.Length() + 1);
    _args[0] = JSValue(_data->Get(context, NewString(isolate, "name")));
    for (int32_t i = 0; i < args.Length(); i++)
        _args[i + 1] = args[i];

    v8::Local<v8::Value> result;

    func->Call(context, o, (int32_t)_args.size(), _args.data()).ToLocal(&result);
...
result_t CefWebView::get_dev(v8::Local<v8::Value>& retVal)
{
    ...
    if (m_domain.size() == 0) {
        ...
        while (s_domains[dom_id][0]) {
            ...
            dev->Set(context, isolate->NewString(dom_name), dom_obj);
            ...
            while (apis[api_id]) {
                ...
                _data->Set(context, isolate->NewString("name"), isolate->NewString(dom_name + '.' + api_name));
                _data->Set(context, isolate->NewString("obj"), o);

                v8::Local<v8::Value> func = isolate->NewFunction(api_name.c_str(), call_method, _data);
                dom_obj->Set(context, isolate->NewString(api_name), func);
