...
result_t util_base::keys(v8::Local<v8::Value> v, v8::Local<v8::Array>& retVal)
{
    ...
    if (v->IsObject()) {
        ...
        if (obj->IsArray()) {
            ...
            for (i = 0; i < len; i++)
                retVal->Set(context, i, isolate->toLocalString(JSValue(retVal->Get(context, i)))).IsJust();
        }
    } else
...
result_t util_base::clone(v8::Local<v8::Value> v, v8::Local<v8::Value>& retVal)
{
    ...
    if (!v->IsProxy() && v->IsObject() && !object_base::getInstance(v)) {
        ...
        if (v->IsFunction() || v->IsArgumentsObject() || v->IsSymbolObject())
            ...
            v8::Local<v8::StringObject> so = v.As<v8::StringObject>();
...
static result_t _deepFreeze(Isolate* isolate, v8::Local<v8::Value> v)
{
    ...
    v8::Local<v8::Value>& retVal)
{
    ...
    for (i = 0; i < argc; i++) {
        ...
        v8::Local<v8::Object> obj1 = val.As<v8::Object>();
    ...
    v8::Local<v8::Object>& retVal)
{
    ...
    for (i = 0; i < argc; i++) {
        ...
        if (o->IsArray()) {
            v8::Local<v8::Array> arr = o.As<v8::Array>();
    ...
    v8::Local<v8::Array>& retVal)
{
    ...
    if (argc > 0) {
        ...
        v8::Local<v8::Array> base = v0.As<v8::Array>();
        ...
        for (i = 1; left > 0 && i < argc; i++) {
            ...
            v8::Local<v8::Array> other = v0.As<v8::Array>();
        ...
        if (left) {
            for (i = 0; i < len; i++)
                if (!erase[i].IsEmpty()) {
                    ...
                    if (j == i)
                        arr->Set(context, n++, erase[i]).IsJust();
                }
        }
    }
...
result_t util_base::first(v8::Local<v8::Value> v, int32_t n, v8::Local<v8::Value>& retVal)
{
    ...
    for (i = 0; i < n; i++)
        arr1->Set(context, i, JSValue(arr->Get(context, i))).IsJust();
...
result_t util_base::last(v8::Local<v8::Value> v, int32_t n, v8::Local<v8::Value>& retVal)
{
    ...
    for (i = 0; i < n; i++)
        arr1->Set(context, i, JSValue(arr->Get(context, len - n + i))).IsJust();
...
result_t util_base::unique(v8::Local<v8::Value> v, bool sorted, v8::Local<v8::Array>& retVal)
{
    ...
    for (i = 0; i < len; i++)
        if (!vals[i].IsEmpty())
            arr1->Set(context, n++, vals[i]).IsJust();
    ...
    v8::Local<v8::Array>& retVal)
{
    ...
    for (i = 0; i < argc; i++) {
        ...
        v8::Local<v8::Array> arr1 = a.As<v8::Array>();
    ...
    v8::Local<v8::Array>& retVal, QuickArray<JSValue>& flatten_list)
{
    ...
    v8::Local<v8::Object> o = list.As<v8::Object>();
    ...
    for (i = 0; i < len; i++) {
        v = o->Get(context, i);
        if (bNext && v->IsObject()) {
            v8::Local<v8::Object> o1 = v.As<v8::Object>();
            v = o->Get(context, isolate->NewString("length"));
            if (IsEmpty(v))
                retVal->Set(context, cnt++, JSValue(o->Get(context, i))).IsJust();
            else {
    ...
    v8::Local<v8::Array>& retVal)
{
    ...
    v8::Local<v8::Object> o = arr.As<v8::Object>();
    JSValue v = o->Get(context, isolate->NewString("length"));
    if (IsEmpty(v))
        return CHECK_ERROR(CALL_E_INVALIDARG);
    ...
    v8::Local<v8::Array>& retVal)
{
    ...
    for (i = 0; i < len; i++) {
        ...
        for (j = 0; j < argc; j++) {
            ...
            v8::Local<v8::Array> without = o.As<v8::Array>();
            len1 = without->Length();
