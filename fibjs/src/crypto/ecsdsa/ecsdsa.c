    ...
    unsigned char* sig, size_t* slen, int (*f_rng)(void*, unsigned char*, size_t), void* p_rng)
{
    if (ctx->grp.id == MBEDTLS_ECP_DP_SECP256K1) {
        ...
        secp256k1_ecsdsa_sign(secp256k1_ctx, signature.data, hash, hlen, &keypair);
    ...
    ret = mbedtls_ecsdsa_sign(&ctx->grp, &r, &s, &ctx->d,
    ...
    const unsigned char* sig, size_t slen, int (*f_rng)(void*, unsigned char*, size_t), void* p_rng)
{
    if (ctx->grp.id == MBEDTLS_ECP_DP_SECP256K1) {
        ...
        secp256k1_reverse_sign(signature.data);
        if (!secp256k1_ecsdsa_verify(secp256k1_ctx, signature.data, hash, hlen, &pubkey))
    ...
    ret = mbedtls_ecsdsa_verify(&ctx->grp, hash, hlen, &ctx->Q, &r, &s, f_rng, p_rng);
    if (ret != 0) {
