...
result_t BlsKey_g1::computeSecret(ECKey_base* publicKey, obj_ptr<Buffer_base>& retVal, AsyncEvent* ac)
{
    if (ac->isSync())
        return CHECK_ERROR(CALL_E_NOSYNC);
    ...
    blst_p1_affine pk;
    mbedtls_mpi_write_binary(&ecp2->Q.X, k, 48);
    blst_p1_uncompress(&pk, k);
