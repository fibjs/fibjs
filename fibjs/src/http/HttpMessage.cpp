...
static bool is_multipart_type(const exlib::string& type)
{
    return qstristr(type.c_str(), "multipart/form-data") != NULL;
}

static bool is_form_type(const exlib::string& type)
{
    return is_multipart_type(type) || qstristr(type.c_str(), "urlencoded") != NULL;
}

static bool has_boundary_param(const exlib::string& type)
{
    return qstristr(type.c_str(), "boundary") != NULL;
}
    ...
    obj_ptr<SeekableStream_base>& retVal, Headers_base* headers, bool defaultFormUrlEncoded)
{
    ...
    if (body->IsString()) {
        ...
        formData = FormData_base::getInstance(body);

        if (formData) {
            // WHATWG Fetch: a FormData body always serializes as multipart/form-data,
            // the boundary generated by the encoder is written back to the header.
            exlib::string type = has_ContentType ? userType : "multipart/form-data";

            hr = formData->encode(type, blob);
            if (hr < 0)
                return hr;
            ...
            if (headers) {
                exlib::string finalType;
                blob->get_type(finalType);
                if (!has_ContentType || (is_multipart_type(finalType) && !has_boundary_param(userType)))
                    headers->set("Content-Type", finalType);
            }
        } else if ((blob = Blob_base::getInstance(body)) != NULL) {
            buf = blob.As<Blob>()->m_impl.getBuffer();
            ...
            hr = GetArgumentValue(isolate, body, converted);
            if (hr < 0)
                return hr;

            exlib::string mimeType;

            if (has_ContentType && !is_form_type(userType))
                mimeType = "application/x-www-form-urlencoded";
            else if (has_ContentType)
                mimeType = userType;
            else
                mimeType = "multipart/form-data";

            hr = converted->encode(mimeType, blob);
            if (hr < 0)
                return hr;
...
result_t HttpMessage::formData(obj_ptr<FormData_base>& retVal, AsyncEvent* ac)
{
    ...
    // can be parsed into a FormData, anything else is a TypeError.
    exlib::string strType;
    ...
    return consumeBody([this, &retVal, strType, is_multipart](result_t n, obj_ptr<Buffer_base> data) -> result_t {
        ...
        if (is_multipart)
            hr = form->parseMultipart(data, strType.c_str());
        else {
            exlib::string strForm;
            data->toString(strForm);
            hr = form->parse(strForm);
        }
