...
void initGlobal(v8::Handle<v8::Object> global)
{
	...
	global->Set(v8::String::New("Buffer"),
			fibjs::Buffer_base::class_info().GetFunction());

	global->Set(v8::String::New("Event"),
			fibjs::Event_base::class_info().GetFunction());

	v8::Local<v8::Object>
			proto =
					global->Get(v8::String::New("Function"))->ToObject() ->GetPrototype()->ToObject();
...
void initMdule()
{
	...
	s_Modules->Set(v8::String::New("assert"),
			assert_base::class_info().CreateInstance());
