...
result_t RangeStream::read(int32_t bytes, obj_ptr<Buffer_base>& retVal, AsyncEvent* ac)
{
    ...
    public:
            ...
            , m_retVal(retVal)
        {
            ...
            if (m_c_pos_snap != m_pThis->real_pos)
                m_pThis->m_stream->seek(m_pThis->real_pos, fs_constants_base::_SEEK_SET);
    ...
    public:
        ...
        {
            result_t hr = m_pThis->m_stream->seek(m_c_pos_snap, fs_constants_base::_SEEK_SET);
...
{
    switch (whence) {
    case fs_constants_base::_SEEK_SET:
        offset += b_pos;
        break;
    case fs_constants_base::_SEEK_CUR:
        offset += real_pos;
        break;
    case fs_constants_base::_SEEK_END:
        offset += e_pos;
        break;
    default:
...
{
    return seek(0, fs_constants_base::_SEEK_SET);
}
