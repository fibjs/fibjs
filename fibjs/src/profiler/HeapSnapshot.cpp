...
result_t HeapSnapshot::load(exlib::string fname)
{
    ...
    hr = GetArray(JSValue(o->Get(isolate->NewString("nodes"))),
        nodes);
    if (hr < 0)
        return CHECK_ERROR(CALL_E_INVALID_DATA);

    hr = GetArray(JSValue(o->Get(isolate->NewString("edges"))),
        edges);
    if (hr < 0)
        return CHECK_ERROR(CALL_E_INVALID_DATA);

    hr = GetArray(JSValue(o->Get(isolate->NewString("strings"))),
        names);
    if (hr < 0)
        return CHECK_ERROR(CALL_E_INVALID_DATA);
    ...
    o = v8::Local<v8::Object>::Cast(v);
    hr = GetArray(JSValue(o->Get(isolate->NewString("node_fields"))),
        node_fields);
    if (hr < 0 || checkArray(node_fields, node_fields_chk, ARRAYSIZE(node_fields_chk)))
        return CHECK_ERROR(CALL_E_INVALID_DATA);

    hr = GetArray(JSValue(o->Get(isolate->NewString("edge_fields"))),
        edge_fields);
    ...
    hr = GetArray(JSValue(v8::Local<v8::Array>::Cast(v)->Get(0)), node_types);
    ...
    hr = GetArray(JSValue(v8::Local<v8::Array>::Cast(v)->Get(0)), edge_types);
