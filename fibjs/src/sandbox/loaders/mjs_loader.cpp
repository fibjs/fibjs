...
public:
    ...
    v8::MaybeLocal<v8::Promise> async_import(exlib::string id, exlib::string base)
    {
        ...
        if (hr < 0) {
            // When hr == CALL_E_JAVASCRIPT, a V8 exception is already pending
            // (e.g. the CJS module threw at runtime). Calling ThrowResult would
            // construct a generic "JavaScript error." and overwrite the real
            // pending exception. Let V8 propagate the original exception instead.
...
private:
        ...
        if (IsEmpty(mod)) {
            ...
            if (type == SandBox::ModuleType::kCommonJS) {
                ...
                if (hr == CALL_E_EXCEPTION) {
                    ...
                    // CALL_E_JAVASCRIPT: a V8 exception is already pending.
                    // Return directly to let it propagate — do NOT read
                    // Runtime::errMessage() which may contain a stale message
                    // from an earlier GetConfigValue call (e.g. "type").
                    return hr;
                }
            }
        }
    ...
    v8::Local<v8::Module> compile_module(exlib::string id, Buffer* data_, v8::Local<v8::Value> exports, bool isCJS = false)
    {
        ...
        if (!IsEmpty(exports)) {
            ...
            // Also treat plain objects with __esModule marker (our CJS interop
            // facade from wrapForCJSRequire) as namespace-like: their default
            // export is obj.default, not the whole object.
            bool isESMNamespace = !isCJS && (exports->IsModuleNamespaceObject()
                || (exports->IsObject() && obj->Has(_context, m_isolate->NewString("__esModule")).FromMaybe(false)));
...
private:
    ...
    static void promise_then(const v8::FunctionCallbackInfo<v8::Value>& args)
    {
        ...
        if (!IsEmpty(_mod)) {
            ...
            // Store the module namespace in mod.exports. When retrieved via
            // wait_module(), the namespace is wrapped with __esModule=true if
            // needed (see SandBox_resolve.cpp). compile_module() also detects
            // the __esModule marker and treats the object as namespace-like.
