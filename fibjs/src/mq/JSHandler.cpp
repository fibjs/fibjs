...
result_t JSHandler::invoke(v8::Local<v8::Object> v, obj_ptr<Handler_base>& retVal,
    AsyncEvent* ac)
{
    if (ac)
        return CHECK_ERROR(CALL_E_NOASYNC);
    ...
    obj_ptr<Message_base> msg = Message_base::getInstance(v);
    v8::Local<v8::Value> a = v8::Local<v8::Value>::New(isolate->m_isolate, v);
    v8::Local<v8::Value> hdlr = GetPrivate("handler");
    ...
    obj_ptr<Handler_base>& retVal, AsyncEvent* ac)
{
    class asyncInvoke : public AsyncEvent {
    public:
        asyncInvoke(Handler_base* pThis, v8::Local<v8::Object>& v,
            obj_ptr<Handler_base>& retVal, AsyncEvent* ac)
            : m_ac(ac)
            , m_pThis(pThis)
            , m_v(v)
