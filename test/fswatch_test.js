...
/**
 * @why disable some test cases about `fs.watchFile()` in low-performance CI environment.
 */
const shouldRunFSWatchFileAboutTest = require('os').cpuNumbers() > 2;
...
describe('fs.watch*', () => {
    ...
    describe("fs.watchFile/fs.unwatchFile", () => {
        ...
        describe("::close", () => {
            ...
            shouldRunFSWatchFileAboutTest && it("robust: allow multiple times close(it's pointless)", () => {
                var triggedCallback = false;
        ...
        shouldRunFSWatchFileAboutTest && it("hold process if last watcher not close", () => {
            var p = process.open(process.execPath, [path.join(__dirname, 'fswatch_test', 'hold2.js')], { timeout: 5000 });
            assert.equal(p.stdout.readLine(), "after start watching");
            assert.equal(p.stdout.readLine(), "watched");
            assert.equal(p.wait(), 9);
        });

        shouldRunFSWatchFileAboutTest && it("hold process if last watcher hasn't been unref-ed", () => {
            var p = process.open(process.execPath, [path.join(__dirname, 'fswatch_test', 'hold3.js'), { timeout: 5000 }]);
            assert.equal(p.stdout.readLine(), "after start watching");
            assert.equal(p.stdout.readLine(), "watched");
            assert.equal(p.wait(), 9);
        });
        ...
        describe("you can watchFile/unwatchFile one non-existed target", () => {
            ...
            var noTriggered = true;

            it("fs.watchFile nil", () => {
                const watcher = fs.watchFile(relfile, { interval: 50 }, () => {
                    noTriggered = false;
                })
        ...
        shouldRunFSWatchFileAboutTest && describe("fs.unwatchFile", () => {
