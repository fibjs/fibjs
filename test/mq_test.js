...
describe("mq", () => {
    ...
    describe("Message arrayBuffer", () => {
        it("should return empty ArrayBuffer for empty message", () => {
            var m = new mq.Message();
            var buf = m.arrayBuffer();
            assert.ok(buf instanceof ArrayBuffer);
            assert.equal(buf.byteLength, 0);
        });

        it("should return ArrayBuffer with correct content after text()", () => {
            var m = new mq.Message();
            m.text("Hello, World!");
            var buf = m.arrayBuffer();
            assert.ok(buf instanceof ArrayBuffer);
        ...
        it("should return ArrayBuffer with correct content after write()", () => {
            var m = new mq.Message();
            m.write(Buffer.from([0x48, 0x65, 0x6c, 0x6c, 0x6f])); // "Hello"
            var buf = m.arrayBuffer();
            assert.ok(buf instanceof ArrayBuffer);
            assert.equal(buf.byteLength, 5);
            var arr = new Uint8Array(buf);
            assert.deepEqual(Array.from(arr), [0x48, 0x65, 0x6c, 0x6c, 0x6f]);
        });

        it("should return ArrayBuffer with binary data", () => {
            var m = new mq.Message();
            m.write(Buffer.from([0x00, 0x01, 0x02, 0xff, 0xfe]));
            var buf = m.arrayBuffer();
            assert.ok(buf instanceof ArrayBuffer);
            assert.equal(buf.byteLength, 5);
            var arr = new Uint8Array(buf);
    ...
    describe("routing handler", () => {
        ...
        it("memory leak 2", () => {
            ...
            var no1 = test_util.countObject('Chain');
            var no2 = test_util.countObject('Routing');
            var no3 = test_util.countObject('Event');

            (() => {
                var data = new coroutine.Event();
                ...
                assert.equal(no1 + 1, test_util.countObject('Chain'));
                assert.equal(no2 + 1, test_util.countObject('Routing'));
                assert.equal(no3 + 1, test_util.countObject('Event'));
            ...
            assert.equal(no1, test_util.countObject('Chain'));
            assert.equal(no2, test_util.countObject('Routing'));
            assert.equal(no3, test_util.countObject('Event'));
        });
    });
