...
declare module 'http' {
     ...
     *      - immutable: 为 true 时在自动生成的 Cache-Control 中附加 immutable 指令，仅当 maxAge 大于 0 时生效
     *      - cacheControl: 是否自动生成 Cache-Control 头，缺省为 true，设为 false 时 maxAge/immutable 不生效
     *      - headers: 按 glob 模式声明响应头，格式为 { pattern: { 头名: 值 } }，按声明顺序匹配，首个命中生效；
     *        模式匹配相对 URL 路径（无前导 /），目录请求自动补 index.html，语法与 path.matchesGlob 相同
     *      @param root 文件根路径
     *      @param options 配置选项
     *      @return 返回一个静态文件处理器用于处理 http 消息
     *      
