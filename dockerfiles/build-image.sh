...
docker build --rm -f linux-build-env.x86 -t fibjs/linux-build-env:x86 .
...
docker build --rm -f alpine-build-env.x86 -t fibjs/alpine-build-env:x86 .
docker build --rm -f alpine-build-env.arm64 -t fibjs/alpine-build-env:arm64 .

docker build --rm -f alpine-test-env.x64 -t fibjs/alpine-test-env:x64 .
docker build --rm -f alpine-test-env.x86 -t fibjs/alpine-test-env:x86 .
...
docker build --rm -f android-build-env.x86 -t fibjs/android-build-env:x86 .
